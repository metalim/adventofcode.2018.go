...
func (f field) draw(start pos, s path) (pos, int) {
	var i int
	p := start
	for ; i < len(s); i++ {
		if p.X < 10 || p.X > DIM-10 || p.Y < 10 || p.Y > DIM-10 {
			_log("warning:", p)
		}
...
func (f field) fill(start pos, m field) int {
...
func (t *task) process() {
	t.m = makeField(DIM)
	t.md = makeField(DIM)
	t.start = pos{DIM / 2, DIM / 2}
	t.m.draw(t.start, t.reg)
	t.max = t.md.fill(t.start, t.m)
}
...
/*
// First I tried to get length without creating a map.
// Problem is we need shortest path from head, but to the longest tail,
// not just shortest path.

func find(s path) (int, int) {
	_log(string(s))
	var cur, i, long int
	short := 1000000000
