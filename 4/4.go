...
func prepare(ss []string) guards {
	gs := guards{}
	var start int
	var g guard

	r := regexp.MustCompile("\\[.*(\\d{2})\\] (\\w+)\\D*(\\d+)?")
...
func main() {
	for i, in := range ins {
		...
		sort.Strings(ss)
		gs := prepare(ss)
		_log("part 1:", part1(gs))
		_log("part 2:", part2(gs))
		_log()
	}
}

var ins = map[string]string{
	"github": `[1518-04-12 00:36] falls asleep
...
[1518-09-03 00:30] wakes up`,
	...
	"google": `[1518-04-22 00:52] wakes up
